...
class Shexchange(feapder.AirSpider):
    ...
    def start_requests(self):
        if not os.path.exists(setting.SH_DOWNLOAD_PATH):
            os.makedirs(setting.SH_DOWNLOAD_PATH)
    ...
    def parse_detail(self, request, response):
        ...
        for e in data:
            ...
            log.info(f'此时的sh_item:{sh_item}')
            sh_item.download_file(e, self.__custom_setting__["BASE_URL"])
            yield sh_item
...
if __name__ == "__main__":
    Shexchange(thread_count=10).start()
